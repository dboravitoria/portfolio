...
export default function CarrosselTecnologias({ categoria }) {
  ...
  return (
    <div className="mb-6 select-none">
      ...
      <Swiper spaceBetween={30} breakpoints={{ 0: {slidesPerView: 1}, 1560: { slidesPerView: 3}}} loop={true} modules={[Navigation]} navigation={{nextEl: ".custom-next", prevEl: ".custom-prev"}} className="mySwiper selection:none">
        ...
        {dadosCategoria.map((item, index) => (
          <SwiperSlide key={index}>
            <div className="bg-white/90 border-2 border-secondaryDark/10  text-secondaryDark p-6 rounded-2xl h-full flex flex-col items-center justify-center text-center backdrop-blur-sm drop-shadow-sm">
              ...
              <h3 className="text-xl font-secondary font-bold selection:none text-secondaryDark capitalize">{item.nome}</h3>
            </div>
          </SwiperSlide>
        ))}
      </Swiper>
